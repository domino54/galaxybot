module.exports = {
	...
	execute: command => {
		...
		if (!command.botGuild.isGalaxyBotManager(command.member)) {
			command.channel.send(`Sorry ${command.user}, you're not permitted to add a ManiaPlanet server status. :no_entry:`);
		...
		if (channelId === undefined || !command.guild.channels.has(channelId)) {
			...
			command.channel.send(`Looks like the the channel for ManiaPlanet servers statuses is not set, ${command.user}. Use \`${prefix}settings servers-status #channel\` to set the channel up!`);
		...
		command.botGuild.log(`Added "${serverLogin}" to ManiaPlanet servers statuses list.`);
