...
module.exports = {
	...
	execute: command => {
		...
		command.botGuild.editSetting(settingName, settingValue, command.galaxybot.config.settings[settingName], command.member).then(setting => {
			...
			if (setting.saved) {
				command.botGuild.log("Saved settings of " + command.guild.name + " to a file: " + setting.saved);
			}

			command.botGuild.log("Showing setting values: " + settingName);
		}).catch(error => {
			var errorMessage;
			...
			switch (error) {
				// Unknown setting.
				case "unknown" :
					errorMessage = "Please provide a valid setting, <@" + command.user.id + ">. The available setting are: ```" + command.botGuild.getSettingsList().join(", ") + "```";
					break;
				...
				case "not boolean" :
					errorMessage = "This setting has to be a boolean, <@" + command.user.id + ">. Use `true` or `false`!";
					break;

				// Incorrect prefix length.
				case "prefix length" :
					errorMessage = "Sorry <@" + command.user.id + ">, but the commands prefix should be between 1 and 5 characters long!";
					break;

				// Duration exceeds the limit.
				case "invalid duration" :
					errorMessage = "The duration you specified doesn't fit the requirements, <@" + command.user.id + ">. Enter a value between `0` and `3600` seconds!";
					break;

				// Setting required providing an action.
				case "incorrect action" :
					errorMessage = "Editing this setting requires you to specify the action you want to make, <@" + command.user.id + ">. Write enter `add` or `remove` before the target value!";
					break;

				// Role not found in the guild.
				case "undefined role" :
					errorMessage = "Sorry <@" + command.user.id + ">, but I couldn't find the role you're trying to set. Make sure it exists and you've spelled its name correctly!";
					break;

				// Target role is above the user's current role.
				case "higher role" :
					errorMessage = "Sorry <@" + command.user.id + ">, but it looks like you're trying to add or remove a role, which is your highest one, or even higher. You can only add or remove roles below your highest one.";
					break;

				// Guild reached roles limit.
				case "too many roles" :
					errorMessage = "Sorry <@" + command.user.id + ">, but it seems like you've reached the roles limit in this server! You can only set up to 10 roles, which can manage me.";
					break;

				// Role already is a GalaxyBot manager.
				case "role already set" :
					errorMessage = "Looks like this role is already able to manage GalaxyBot, <@" + command.user.id + ">.";
					break;

				// Role is not a GalaxyBot manager.
				case "role not set" :
					errorMessage = "This role doen't have permissions to manage GalaxyBot, <@" + command.user.id + ">.";
					break;

				// Channel not found in the guild.
				case "undefined channel" :
					errorMessage = "Sorry <@" + command.user.id + ">, but I couldn't find the channel you're trying to set. Make sure it exists, belongs to this server and you mentioned it properly!";
					break;

				// Guild reached channels limit.
				case "too many channels" :
					errorMessage = "Sorry <@" + command.user.id + ">, but it seems like you've reached the channels limit for this setting! You can only set up to 10 channels, where music comands are whitelisted.";
					break;

				// Channel is already whitelisted.
				case "channel already set" :
					errorMessage = "Looks like this music player commands are already whitelisted in this channel, <@" + command.user.id + ">.";
					break;

				// Channel is not whitelisted.
				case "channel not set" :
					errorMessage = "Music player commands are not whitelisted in this channel, <@" + command.user.id + ">.";
					break;
				...
				// Guild reached channels limit.
				case "too many words" :
					errorMessage = "Sorry <@" + command.user.id + ">, but it seems like you've reached the maximum number of filtered words (" + command.galaxybot.config.filter.count + ").";
					break;

				// Channel is already whitelisted.
				case "word already set" :
					errorMessage = "This word is already set in the filter, <@" + command.user.id + ">.";
					break;

				// Channel is not whitelisted.
				case "word not set" :
					errorMessage = "This word is not filtered, <@" + command.user.id + ">.";
					break;
